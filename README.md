...
| SE Workbench Migration Plan | v0.6.1 (draft) | `migration-plans/se-workbench/PKM_MIGRATION_PLAN.md` |
...
| UDM v2.0 — Automated SEMP Generation Architecture (`proposals/UDM_V2_SEMP_GENERATION_PROPOSAL.md` v0.6.1) | Architecture Guidance §11–§12 shipped; PKM `Role`/`RiskItem` (structure, data, and SEMP-wiring) all shipped and verified — 3 of 4 data-integrity findings resolved same-day, Step 11 (`baselineId`/`projectId`) genuinely unresolved pending a fresh export | **Draft** — grounding source (§6 item 1) still open (DI-SESS-81785B/SEP Outline v4.1 may supersede 24748-4). New §6 item 8: static-deploy caching limitation, candidate Architecture Guidance addition, not yet drafted. |
