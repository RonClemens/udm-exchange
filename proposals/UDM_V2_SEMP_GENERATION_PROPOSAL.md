...
**Version:** 0.6.0 (Exploratory / Proposal — Role, RiskItem, and Phase C all real and implemented; four data-integrity gaps found via real export cross-check; grounding-source question reopened, not settled)
**Last updated:** 2026-07-30
**Status:** Draft
**Depends on:** PKM Entity Model v0.6.0, Architecture Guidance v1.5.0, SE Workbench Migration Plan v0.6.0
...
| **D** | Extend structured generation to deliverables, RACI (`Role`/`RiskItem` both now exist), technical scope (CI/Requirement tree) | Moderate | **Partially approved** — the `RiskItem` slice specifically, to fill §3.2.1 (currently disclaiming "no dedicated risk-register entity" despite `RiskItem` existing and being fully populated). Same low-risk pattern as `buildMilestoneSchedule()`: pure query-and-format, zero AI calls. RACI and deliverables-list slices not yet approved — narrower scope than the full Phase D as originally sketched. |
...
## 6. Open items — status as of 2026-07-30

1. **Grounding source — reopened, not just "still open."** The generated SEMP package cites DI-SESS-81785B (the actual SEMP DID) and OSD SEP Outline v4.1 directly, with real source URLs. This may be a better, more directly applicable grounding source than ISO/IEC/IEEE 24748-4 for this exact deliverable — worth deciding whether 24748-4 access is still needed at all before continuing to treat it as the blocking item, or whether design chat should shift to grounding Phase A/E against DI-SESS-81785B + SEP Outline v4.1 instead.
...
3. ~~`RiskItem` vs. `Gap` boundary~~ **Design resolved and the field exists** (`Gap.escalatedToRiskItemId`, PKM v0.6.0) — **but real-data population is unverified and, per today's export cross-check, actually absent** on the one record that should have it (`gap-003`). Migration Plan v0.6.0 Step 14 backfills this. Correcting an overclaim from v0.5.2, which read this as fully "implemented" without checking real data.
4. **Risk Management Board — still parked, not resolved.** (§3.1) "No PKM entity, for now" — explicitly provisional pending a separate risk-planning session Ron intends to hold.
5. **Whether generated SEMP output itself needs any PKM-level audit trail — still not yet.** Left open, not decided either way.
6. ~~`RiskItem` itself — not yet implemented~~ **Structurally shipped and verified** (Workbench's own Step 12, CRUD route, `RiskItemsPage.tsx`, all three `itemType` values seeded) — **but its §3.2.1 SEMP-generation wiring is not yet done** (§4 Phase D, partially approved above) and its cross-references to `Gap`/`ActionItem` are unpopulated on legacy records (item 3 above, Step 14). Correcting the same overclaim pattern as item 3.
7. **New, from today's cross-check (2026-07-30):** four real data-integrity gaps found via a live backend export, not prose review — Migration Plan v0.6.0 Steps 11–14 (baseline FK backfill, `ReconciliationEvent` migration, `milestoneType` backfill, Gap/ActionItem reference backfill). Step 12 corrects a genuine planning error on design chat's part: PKM v0.4.0 assumed `Baseline`'s reconciliation fields were never populated and removed them on that basis, without ever checking real data — they were populated, and no migration step existed to move that data before now.
8. **Migration plan and coaching commands — status.** Steps 9–14 all drafted; 9–10 confirmed relayed and implemented, 11–14 newly drafted and going out now. Architecture Guidance §11–§12 both drafted and live. Phase C is complete; Phase D partially approved (RiskItem→§3.2.1 only). What remains genuinely undrafted: Phase A's template-mapping document and Phase E/F's narrative-generation guidance — grounding source for both now an open question (item 1).

---

*Grounding source (item 1) is an open question again, not a settled blocker — DI-SESS-81785B/SEP Outline v4.1 may already be sufficient. Items 3, 4, 6, and 7 involve real but low-risk backfill work now queued as Migration Plan Steps 11–14. Role, Phase C, and RiskItem's structure are all implemented and verified; RiskItem's data population and SEMP-generation wiring are not yet complete.*
