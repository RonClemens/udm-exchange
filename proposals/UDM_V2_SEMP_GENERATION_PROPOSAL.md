...
**Version:** 0.6.1 (Exploratory / Proposal — Role, RiskItem, Phase C, and Phase D's RiskItem slice all real, implemented, and data-verified; grounding-source question still open)
**Last updated:** 2026-07-30
**Status:** Draft
**Depends on:** PKM Entity Model v0.6.0, Architecture Guidance v1.5.0, SE Workbench Migration Plan v0.6.1
...
7. ~~Four data-integrity gaps from the 2026-07-30 export cross-check~~ **Three resolved same-day** (Steps 12, 14). **One (`baselineId`/`projectId`, Step 11) remains genuinely open** — Workbench's real data has it populated everywhere checked; the original finding was almost certainly against a stale static-demo build, but this isn't fully confirmed. A fresh export against `mock-data/seed.json` would settle it.
...
*Grounding source (item 1) is still open — DI-SESS-81785B/SEP Outline v4.1 may be sufficient, not yet decided. Role, RiskItem (structure, data, and generation wiring), and Phase C are all implemented and verified. Only Step 11 (baselineId/projectId) remains a genuinely open, unconfirmed question — likely a stale-export artifact, not a real gap, pending a fresh check.*
