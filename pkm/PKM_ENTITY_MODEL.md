...
**Version:** 0.5.0 (Exploratory / Draft — not yet implemented in any app)
...
| **ActionItem** | A remediation task | resolves Gap; assigned to a Role via `assignedRoleId` (see new Role row) — not a named person, by default |
...
| **Role** | A named responsibility/authority position within a program's SE organization | belongs to Project; `name`, optional `authorityDescription` (SEMP-facing content), `isDefault` (distinguishes the standard starting taxonomy from program-added custom roles); referenced by `ActionItem.assignedRoleId` |
